...
#             if not result:
#                 # no events → normal idle
#                 self.flush_aggregates()
#                 time.sleep(0.1)
#                 continue

#             _, event_id = result
#             print(f"[aggregation-worker] Processing event {event_id}")
...
#             try:
#                 data = json.loads(raw)
#                 # If it's a batch pushed by the main worker, skip — wrong queue
#                 if isinstance(data, dict):
#                     print(f"[aggregation-worker] Skipping non-integer payload: {raw[:80]}")
#                     self.flush_aggregates()
#                     continue
#                 # If JSON but a plain number
#                 event_id = int(data)
#             except (json.JSONDecodeError, ValueError):
#                 # Plain string like "123"
#                 event_id = int(raw)
