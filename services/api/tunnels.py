...
def list_tunnels(user_id: str = Depends(get_current_user)):
    ...
    try:
        tunnels = db.execute(
            text("""
                SELECT 
                    id, name, local_url, public_url, status,
...
def create_tunnel(
    ...
    try:
        ...
        # Format: https://hook-{random}.hooktrace.dev
        random_id = secrets.token_urlsafe(8).replace('_', '').replace('-', '')[:8]
        public_url = f"https://hook-{random_id}.hooktrace.dev"
        
        db.execute(
            text("""
                INSERT INTO dev_tunnels 
                (id, user_id, name, local_url, public_url, status)
                VALUES (:id, :user_id, :name, :local_url, :public_url, 'active')
            """),
            {
                "id": tunnel_id,
                "user_id": user_id,
